...
- 用户确认：2026-08-12 已明确确认当前 006 方案并要求实施。
- 当前阶段：实施完成，待创建任务提交。
- 实施门禁：已满足；当前允许执行 `APP-000` 至 `VER-001`。
...
- 方案文档已在 `3e4d668` 独立提交并推送；实施、测试和随真实代码变化产生的文档更新作为一个完整任务变更审阅与提交；未经用户要求不 push。
...
- [x] **CMP-002（3 点）** 为 Clock、ID Generator、Validator 建立 Fixed Direct app Definition 和契约测试。
  - 依赖：APP-003。
  - 验收：输出分别为现有项目接口；无 Access.Use、ConfigPath、Defaults、CLI 或生命周期；实现替换契约测试通过。
- [x] **CMP-003（4 点）** 重构 composition 为本地 Plan、Freeze、Defaults/CLI 校验和最后原子 Install。
...
- [x] **CLN-001（2 点）** 删除旧 capability 目录、Definition/Registration/Handle/Access/InstanceHooks API 和全部旧引用。
  - 依赖：CMP-003。
  - 验收：旧路径、类型、import、测试、说明和逐项 Register 搜索归零；不保留 alias、legacy 或兼容分支。
- [x] **DOC-002（1 点）** 同步根 README、docs 入口、Kernel/App 与相关 pkg 权威说明，并更新 002 研究状态边界。
  - 依赖：CLN-001。
  - 验收：当前实现与后续 Native/Handoff/观察期目标明确分开；示例与真实 API 一致；不设计业务层。

### 验证与提交

- [x] **VER-001（2 点）** 执行完整验证、审阅最终 Diff 并创建单一 006 实施 commit。
...
| --- | --- | --- | ---: | ---: | --- | --- | --- |
| 1 | 2026-08-12 | DOC-001 | 1 | 1 / 44 | 006 四件套、变更索引、UTF-8、Markdown 相对链接、范围检查和 `git diff --check` | `3e4d668` | APP-000 至 VER-001 尚未实施。 |
| 2 | 2026-08-12 | APP-000 至 DOC-002 | 41 | 42 / 44 | Go 1.25.7 临时泛型探针通过且未保留；定向测试与 `go test ./...` 通过；最终验证待 VER-001 记录。 | 待提交 | Native Reload、Handoff、观察期与自动回切不在 006；未连接真实 Database。 |
| 3 | 2026-08-12 | VER-001 | 2 | 44 / 44 | `gofmt`、`go mod tidy` 无差异、build、全量测试、race、vet、实际 `config init`、68 个 Markdown 链接、旧轨/自动发现搜索和 diff check 全部通过。 | 本次提交 | 未连接真实 Database；生成配置已检查并删除。 |
