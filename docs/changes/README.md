...
- [031 模块顶层 HTTP Handler 分责](031-module-top-level-http-handler/README.md)：研究与计划已完成，待确认；拟把 Todo 模块 HTTP handler 层从 `binding/http` 迁移到模块顶层 `handler`，使 binding 只做代码优先契约与运行期装箱，每层职责分明。
