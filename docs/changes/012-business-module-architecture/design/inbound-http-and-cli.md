...
### 3.2 Route/Module policy Middleware
...
当前 `config init` 保留 Bootstrap 语义。Application 命令作为 one-shot Participant 或等价受管任务，在 Kernel 和必要 Module 就绪后执行；完成、取消或失败都触发统一反向停止。

## 8. 验证要求

- Handler 测试只使用 Service stub，覆盖解析、校验、取消、错误映射和 I18n。
