...
- 当前有效的架构、技术栈和开发流程分别放在对应的 `docs/` 文档中，不向本文件堆叠。
- 新增需求、架构调整或跨模块实施任务必须在 `design/<seq-num-name>/**` 中建立任务级文档，其中 `<seq-num-name>` 使用递增序号和语义名称；至少包含面向产品的需求、面向开发者的设计（具体实现方式、技术细节、接口和文件改动）以及面向 Agent 的任务清单。任务清单必须记录稳定任务 ID、工作量、完成条件，并在每轮执行后标记本次完成的任务量和验证证据。
- `design/` 描述目标和实施过程；实现完成后必须把仍然有效的结论同步到当前权威文档，不能让已完成设计长期成为第二套现行规范。
