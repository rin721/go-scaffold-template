...
- `<seq-num-name>` 使用仓库内递增三位序号和语义名称。每项变更固定包含 `README.md`、`requirements.md`、`design.md` 和 `tasks.md`：
...
除纯文档交付和无副作用的只读分析外，以下事项一律进入确认门禁：

- 新增、修改或删除源码、配置、脚本、依赖声明、生成物和测试实现。
- 执行会改变工作区、Git、进程、数据库、部署环境或外部系统状态的命令，包括生成、迁移、启动、停止、部署和外部写入。
- 用户初始请求即使包含“实现”“直接执行”“启动项目”等措辞，也只授权进入方案阶段，不能同时视为对尚未形成方案的确认。

不修改仓库和外部状态的读取、搜索、静态检查、构建或测试可以用于补充方案事实，但不得借此提前实施任务。

### 4.2 方案阶段

1. 从根 `README.md` 和当前代码事实开始分析，在 `docs/changes/<seq-num-name>/` 建立完整任务文档，并把状态标为“待确认”。
2. 方案阶段只允许修改该任务文档和必要的文档导航；不得修改实现文件，不得执行状态变更命令，也不得暂存或提交方案文档。
3. Agent 必须向用户提交需求报告，明确目标、验收标准、范围、接口与文件影响、风险、未决项和任务清单，然后停止工作并等待确认。
4. 方案报告之前的请求、其他任务的确认、历史消息中的概括授权都不能跨任务复用。

### 4.3 确认与实施阶段

- 只有用户在方案报告之后的后续消息中，明确确认当前任务及当前方案，才能把任务状态改为“已确认”并开始实施。
- 用户要求调整方案时，先更新任务文档并重新提交报告；不得把修改意见本身视为实施确认。
- 需求目标、公共接口、依赖选择、模块边界、数据迁移或外部副作用发生实质变化时，必须回到“待确认”状态并重新获得确认。已确认范围内的普通实现细节不重复确认。
- 实施只能覆盖已确认的任务 ID。发现范围外问题时记录风险，不顺手实施。
- 用户已选择方案阶段不单独提交：确认后，任务文档、实现、测试和权威文档同步结果作为同一任务变更提交；用户明确要求不提交时除外。
- 纯文档任务如果最终交付物只有文档、规则或文档目录调整，可以完成并提交，不需要人为制造第二次确认；一旦包含非文档实现，必须使用完整门禁。
...
- 执行非纯文档变更前，必须确认当前任务目录存在、方案状态为“已确认”，且本轮动作属于已确认任务 ID；任一条件不满足都返回方案阶段。
...
- 文档声称的能力在代码中尚不存在。
- 未建立任务文档、未获得当前方案确认或越过确认范围就开始非文档实施。
