...
4. 每个变更至少建立一份 `research/Rxxx-<semantic-name>/metadata.yaml` 与 `report.md`。记录必须包含研究问题、证据定位与快照、事实、推断、适用与不适用场景、局限、有效期或刷新触发器，以及对本任务的影响；不得把框架能力、目标方案或个人判断写成项目已实现事实。
5. 研究门禁只有在关键问题已有可复核证据、事实与推断明确分离、冲突证据已经解释、剩余未知不妨碍形成计划时才能标记通过。证据不足、易漂移事实未复核或关键边界未知时，不得进入计划阶段。
