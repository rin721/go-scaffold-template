...
func TestServiceCreateListAndComplete(t *testing.T) {
	...
	created, err := service.Create(t.Context(), CreateCommand{Title: "  学习 Go  "})
	if err != nil || created.Title != "学习 Go" || created.Status != model.StatusPending || created.Version != 1 {
		t.Fatalf("Create() = %#v, %v", created, err)
	}
	listed, err := service.List(t.Context(), ListQuery{})
	if err != nil || listed.Total != 1 || listed.Limit != 10 || len(listed.Items) != 1 {
		t.Fatalf("List() = %#v, %v", listed, err)
	}
	completed, err := service.Complete(t.Context(), CompleteCommand{ID: testID})
	if err != nil || completed.Status != model.StatusCompleted || completed.Version != 2 {
		t.Fatalf("Complete() = %#v, %v", completed, err)
	}
	repeated, err := service.Complete(t.Context(), CompleteCommand{ID: testID})
...
func TestServiceClassifiesInvalidInputAndRepositoryErrors(t *testing.T) {
	...
	for _, call := range []func() error{
		func() error { _, err := service.Create(t.Context(), CreateCommand{Title: "long"}); return err },
		func() error { _, err := service.Get(t.Context(), GetQuery{ID: "invalid"}); return err },
		func() error { _, err := service.List(t.Context(), ListQuery{Limit: 3}); return err },
		func() error { _, err := service.List(t.Context(), ListQuery{Status: "unknown"}); return err },
	} {
		if err := call(); fault.CodeOf(err) != fault.CodeInvalidArgument {
			t.Fatalf("error = %v, code = %s", err, fault.CodeOf(err))
		}
	}
	_, err = service.Get(t.Context(), GetQuery{ID: testID})
	if fault.CodeOf(err) != fault.CodeNotFound {
		t.Fatalf("Get(missing) error = %v, code = %s", err, fault.CodeOf(err))
	}
	cancelled, cancel := context.WithCancel(t.Context())
	cancel()
	if _, err := service.List(cancelled, ListQuery{}); !errors.Is(err, context.Canceled) {
...
func TestServicePreservesDependencyFailures(t *testing.T) {
	...
	serviceWithIDFailure, err := New(newMemoryRepository(), clock.Fixed(now), fixedIDs{err: idFailure}, policy)
	if err != nil {
		t.Fatalf("New(id failure) error = %v", err)
	}
	if _, err := serviceWithIDFailure.Create(t.Context(), CreateCommand{Title: "学习 Go"}); fault.CodeOf(err) != fault.CodeInternal || !errors.Is(err, idFailure) {
		t.Fatalf("Create(id failure) error = %v", err)
	}

	serviceWithInvalidID, err := New(newMemoryRepository(), clock.Fixed(now), fixedIDs{id: "invalid"}, policy)
	if err != nil {
		t.Fatalf("New(invalid ID) error = %v", err)
	}
	if _, err := serviceWithInvalidID.Create(t.Context(), CreateCommand{Title: "学习 Go"}); fault.CodeOf(err) != fault.CodeInternal {
		t.Fatalf("Create(invalid generated ID) error = %v", err)
	}

	serviceWithZeroClock, err := New(newMemoryRepository(), clock.Fixed(time.Time{}), fixedIDs{id: testID}, policy)
	if err != nil {
		t.Fatalf("New(zero clock) error = %v", err)
	}
	if _, err := serviceWithZeroClock.Create(t.Context(), CreateCommand{Title: "学习 Go"}); fault.CodeOf(err) != fault.CodeInternal {
	...
	serviceWithRepositoryFailure, err := New(failingRepository{err: repositoryFailure}, clock.Fixed(now), fixedIDs{id: testID}, policy)
	if err != nil {
		t.Fatalf("New(repository failure) error = %v", err)
	}
	if _, err := serviceWithRepositoryFailure.Get(t.Context(), GetQuery{ID: testID}); fault.CodeOf(err) != fault.CodeUnavailable || !fault.Retryable(err) || !errors.Is(err, repositoryFailure) {
