...
HTTP binding 不属于 module core，但属于 Todo 模块。Todo 用代码优先方式声明自己的公开契约：`binding/http/contract.go` 与 `contract_module.go` 使用 `pkg/httpx/contract` 描述 operation、policy、security 与 DTO schema；`dto.go` 定义模块自有 HTTP DTO；`handler.go` 实现窄 `Operations` 接口；`handlers.go` 把 typed handler 装箱为运行期执行器。`internal/tools/contract-gen` 据此生成 `api/openapi.yaml` 与 operation inventory。Todo 不创建 Chi Router、不加载 OpenAPI、不执行 operation policy、不满足整份应用接口；生成类型不会进入 model、service 或 repo。
