...
`internal/module/ops` 在独立 management listener 提供 `/startupz`、`/livez`、`/readyz`、`/metrics`、`/build` 和受 `management:read` 保护的 `/diagnostics`；业务 listener 不暴露这些路径，pprof 不注册。`pkg/observability` 定义项目自有契约，Prometheus/OTel/OTLP 只存在于 `internal/kernel/app/observability`；Prometheus registry 由进程稳定持有，OTel provider/exporter 随 generation 构造、请求排空并有界 flush。GenerationCoordinator 继续提供 attempt、candidate/current/retiring generation、Snapshot digest、changed sections、phase、地址、活动连接/请求、资源 build/reuse、cleanup debt 和脱敏失败类型；当前仍不提供跨进程 retry/force CLI。

[027 第三方封装与分轨装配](docs/changes/027-business-module-third-party-isolation/README.md) 已实现第三方分轨：新增业务能力先完整收口到 `internal/module/<name>`，业务专属第三方留在模块 Adapter 并零泄漏；只有跨业务复用且由进程统一选择的资源才进入完整底层装配链。Architecture/AST 门禁会拒绝模块非 Adapter 的第三方 import、导出 selector 泄漏、composition 穿透模块 Adapter 和 `pkg`/Kernel App 导出第三方类型。
